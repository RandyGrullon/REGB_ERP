...
**Que resuelve:** el 18% deja de estar cableado y la retencion del
proveedor deja de calcularse en una hoja aparte. Encima, la liquidacion
IT-1 del mes y el calendario de vencimientos.
...
de `tax` en la accion y la tabla lo verifica con un check
(`tax <> 'itbis' or base = 'itbis'`): ofrecerlo como opcion es ofrecer la
...
como se cobra un 1800% de ITBIS. La pantalla acepta que escribas "18" o
"0.18" —la gente escribe las dos— pero guarda siempre la fraccion.
...
`packages/operations/src/taxes.ts`: `TASAS_ITBIS_RD`, `VENCIMIENTOS_RD`,
`proximoDiaHabil`, `calendarioFiscal`, `desglosarItbis`,
`calcularRetenciones` y `liquidarItbis`. Cuatro razones, no una:
...
- **Permisos:** `view`, `rate.manage`, `rule.manage`, `profile.assign`,
  `filing.close`, `export`
- **Widgets:** `taxes-next-due`, `taxes-itbis-due` *(declarados; sin
  pintar todavia)*
...
| 2 | Migraciones + RLS probadas | ✅ `supabase/tests/taxes.test.ts` — 24 casos: aislamiento en las 4 tablas, escritura con `tenant_id` ajeno, modulo apagado (lectura y escritura), el trigger por triplicado, el trigger **en UPDATE**, y 12 restricciones de tabla |
| 3 | Logica pura con cobertura | ✅ `packages/operations/src/taxes.test.ts` — 30 pruebas: fin de semana al lunes, diciembre rodando a enero, dias restantes, desglose que cuadra al centavo, base de ITBIS vs base de ISR, comodin `ambas`, exento, y saldo a favor que se arrastra |
| 4 | UI web responsive | ⚠️ 4 pantallas escritas y con `tsc` limpio, pero **sin abrir en navegador**: el modulo todavia no esta registrado en `bootstrap.ts` (lo engancha quien coordina) |
...
| 9 | ≥2 widgets | ⚠️ declarados en el manifiesto, **sin pintar**: `components/widgets.tsx` es de quien coordina |
