...
export async function registrarFactura(fd: FormData): Promise<ActionResult> {
  ...
  const supplierNcf = String(fd.get('supplierNcf') ?? '').trim() || null
  const subtotal = num(String(fd.get('subtotal') ?? ''))
  const tax = num(String(fd.get('tax') ?? '0')) ?? 0
  const retencion = num(String(fd.get('retention') ?? '0')) ?? 0
  ...
  if (retencion < 0) return { ok: false, error: 'La retencion no puede ser negativa.' }
  ...
  try {
    const res = await asUser(ctx.userId, ctx.tenantId, async (tx) => {
      ...
      const emision = issueDateRaw ? new Date(`${issueDateRaw}T12:00:00`) : new Date()
