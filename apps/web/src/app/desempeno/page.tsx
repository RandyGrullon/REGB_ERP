    ...
    <Shell {...shell} activePath="/desempeno">
      <div className="space-y-5">
        <PageHeader
          icon="trending_up"
          title="Desempeno"
          description="El progreso de cada OKR se deriva del valor actual contra la meta, nunca un numero que alguien actualiza aparte."
        />

        <section aria-label="Resumen" className="grid grid-cols-2 gap-3 lg:grid-cols-3">
          <StatCard label="Objetivos activos" value={String(objetivos.filter((o) => o.status === 'active').length)} />
            ...
            {objetivos.length === 0 ? (
              ...
              objetivos.map((o) => {
                ...
                const progreso = progresoObjetivo(
                  krs.map((k) => ({ current: Number(k.current_value), target: Number(k.target_value) })),
                )
                return (
                  <div key={o.id} className="border-b border-[var(--color-border-subtle)] pb-3 last:border-0">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge tone={badgeTono(o.status)}>{ESTADO_OBJETIVO[o.status] ?? o.status}</Badge>
                      </div>
                    </div>
                    {krs.length > 0 && (
                      <ul className="mt-2 space-y-1">
                        {krs.map((k) => (
                          <li key={k.id} className="flex items-center justify-between gap-2 text-xs">
                            <span className="text-[var(--color-text-secondary)]">{k.description}</span>
                            <span className="flex items-center gap-2">
                              ...
                              <span className="tabular text-[var(--color-text-primary)]">
                                {progresoResultadoClave(Number(k.current_value), Number(k.target_value))}%
                              </span>
                              {puedeObjetivos && (
                                <form action={actualizarProgresoForm} className="flex items-center gap-1">
                    ...
                    {puedeObjetivos && (
                      <form action={crearResultadoClaveForm} className="mt-2 flex flex-wrap items-end gap-2">
                        <input type="hidden" name="tenant" value={qs ? ctx.tenantSlug : ''} />
                        <input type="hidden" name="rol" value={qs ? ctx.roleName : ''} />
                ...
                <TBody>
                  {unoAUnos.map((u) => (
                    <TR key={u.id}>
                      ...
                      <TD>
                        <Badge tone={badgeTono(u.status)}>{ESTADO_1ON1[u.status] ?? u.status}</Badge>
                      </TD>
                    ...
                    <TR key={e.id}>
                      ...
                      <TD numeric>
                        <span className="tabular font-semibold text-[var(--color-text-primary)]">
                          {promedioEvaluacion360(evaluacionesPorCiclo.get(`${e.employee_name}·${e.cycle}`) ?? [])}
                        </span>
                      </TD>
                    </TR>
                  ))}
            ...
            {puedeEvaluar && empleados.length > 0 && (
              <form action={enviarEvaluacionForm} className="flex flex-wrap items-end gap-3">
                ...
                <label className="flex w-20 flex-col gap-1 text-xs text-[var(--color-text-muted)]">
                  Calificacion (1-5)
                  <input name="rating" inputMode="numeric" required className={`tabular ${claseInput}`} />
                </label>
        ...
        <Card>
          ...
          <CardBody className="space-y-3">
            {planes.length === 0 ? (
              <EmptyState icon="assignment" title="Todavia no hay ningun plan de mejora" description="" />
            ) : (
              <Table>
                <THead>
                  <TR>
                ...
                <TBody>
                  {planes.map((p) => (
                    <TR key={p.id}>
                      ...
                      <TD>{p.end_date}</TD>
                      <TD>
                        <Badge tone={badgeTono(p.status)}>{ESTADO_PLAN_MEJORA[p.status] ?? p.status}</Badge>
                      </TD>
                      {puedePlanes && (
                        <TD>
                          {p.status === 'active' && (
