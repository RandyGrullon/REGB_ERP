...
import { FORMAS_PAGO_606, TIPOS_ANULACION, TIPOS_GASTO_606, periodoFiscal } from '@regb/operations'
 ...
 *
 * La comparten Por cobrar (/cobrar/dgii) y la Caja (/pos/dgii): un colmado
 * sin credito tambien declara su 607. Una sola pantalla para que no haya
 * dos verdades sobre lo que se le declara a la DGII.
 */
...
export async function ReportesDgii({
  ctx,
  periodoPedido,
  veCompras,
  rutaBase,
  ...
  ctx: ModulePageCtx
  periodoPedido: string | undefined
  /**
   * El 606 sale de COMPRAS: se enseña solo a quien puede exportar `ap`.
   * Quien ve lo que el negocio factura no tiene por que ver lo que compra.
   */
  veCompras: boolean
  /** La ruta de ESTA pantalla, para limpiar el filtro sin salir de ella. */
  rutaBase: string
  crumbs: { label: string; href?: string }[]
}) {
  // Por defecto el mes corriente EN RD, que es el que se esta armando. Con
  // getMonth() del servidor (UTC), desde las 8 p. m. del ultimo dia ya
  // proponia el mes siguiente.
  const periodoActual = periodoFiscal(new Date())
  const periodo = /^\d{6}$/.test(periodoPedido ?? '') ? periodoPedido! : periodoActual

  const [compras, ventas, anulados, periodos, ocultas] = await asUser(
    ctx.userId,
    ctx.tenantId,
    async (tx) => {
      const c = veCompras
      ...
      // Ventas del periodo que un modulo apagado esconde (0129). Si hay, el
      // 607 de abajo esta CORTO y parece completo: se dice, y el TXT no sale.
      const o = await tx<{ modulo: string; documentos: string }[]>`
      select modulo, documentos::text from public.ventas_fuera_de_vista(${periodo})`
      return [c, v, a, p, o] as const
    },
  )

  // Una nota de credito (B04) viene en positivo, como la pide el 607, y
  // RESTA de las ventas del periodo (0130).
  const signo = (f: { origen: string }) => (f.origen === 'nota_credito' ? -1 : 1)
  const totalVentas = ventas.reduce((s, f) => s + signo(f) * Number(f.total), 0)
  const totalItbis = ventas.reduce((s, f) => s + signo(f) * Number(f.itbis_facturado), 0)
  ...
  const escondidas = ocultas.reduce((s, o) => s + Number(o.documentos), 0)
  ...
  return (
    <div className="space-y-5">
      ...
      {escondidas > 0 && (
        ...
        >
          ...
          <p className="text-[var(--color-text-secondary)]">
            Hay <strong className="text-[var(--color-text-primary)]">{escondidas}</strong> venta
            {escondidas === 1 ? '' : 's'} de este periodo en un modulo que esta apagado (
            {ocultas.map((o) => o.modulo).join(', ')}). El 607 de abajo{' '}
            <strong className="text-[var(--color-text-primary)]">no las incluye</strong> y el TXT no
            se genera hasta volver a encenderlo: declarar de menos es una multa.
          </p>
        </div>
      )}
      ...
      <Toolbar hidden={qs ? { tenant: ctx.tenantSlug, rol: ctx.roleName } : {}}>
        ...
        <ToolbarActions hasFilters={periodo !== periodoActual} clearHref={`${rutaBase}${qs}`} />
      </Toolbar>
      ...
      >
        ...
        <p className="text-[var(--color-text-secondary)]">
          ...
          <strong className="text-[var(--color-text-primary)]">declara en el 608</strong>, nunca se
          omite: para la DGII un hueco en la secuencia es una alerta.
        </p>
      </div>
        ...
        <>
          {comprasSinClasificar > 0 && (
            ...
            >
              ...
              <p className="text-[var(--color-text-secondary)]">
                Hay{' '}
                <strong className="text-[var(--color-text-primary)]">{comprasSinClasificar}</strong>{' '}
            ...
            <TBody>
              {anulados.map((f) => (
                <TR key={f.ncf}>
                  ...
                  <TD>
                    <span className="text-xs text-[var(--color-text-muted)]">{f.explicacion}</span>
                  </TD>
                </TR>
              ))}
            </TBody>
